...
export default async function StudyPage() {
  ...
  if (study.status === "not_started") await db().execute({ sql: "UPDATE studies SET status='in_progress',started_at=?,updated_at=? WHERE participant_id=?", args: [now(), now(), user.userId] });
  const interactions = (await db().execute({ sql: "SELECT sequence_no,full_prompt,ai_response,created_at FROM interactions WHERE participant_id=? ORDER BY sequence_no", args: [user.userId] })).rows;
