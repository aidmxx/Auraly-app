...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{created?:string;deleted?:string}> }) {
  ...
  return <main className="app-shell"><header className="topbar"><div><strong>Auraly Research Console</strong><span>Signed in as {admin.loginId}</span></div><form action={logout}><button className="link-button">Sign out</button></form></header>
    ...
    <section className="panel"><div className="section-head"><div><h2>Participant progress</h2><p className="muted">Deleting a participant permanently removes their account and all associated study data.</p></div><div className="export-links"><a href="/api/admin/export?format=csv">Export CSV</a><a href="/api/admin/export?format=json">Export JSON</a></div></div><div className="table-wrap"><table><thead><tr><th>ID</th><th>Condition</th><th>Status</th><th>Started</th><th>Submitted</th><th>Time</th><th>Words</th><th>AI uses</th><th>Drafts</th><th>Account</th></tr></thead><tbody>{rows.map(r=><tr key={String(r.id)}><td><strong>{String(r.login_id)}</strong></td><td>{String(r.condition_code)}</td><td><span className={`status ${r.status}`}>{String(r.status).replace("_"," ")}</span></td><td>{r.started_at?new Date(String(r.started_at)).toLocaleString():"—"}</td><td>{r.submitted_at?new Date(String(r.submitted_at)).toLocaleString():"—"}</td><td>{r.completion_seconds?`${Math.round(Number(r.completion_seconds)/60)} min`:"—"}</td><td>{Number(r.final_word_count)||"—"}</td><td>{Number(r.interactions)}</td><td>{Number(r.drafts)}</td><td><DeleteParticipantButton userId={String(r.id)} loginId={String(r.login_id)} /></td></tr>)}</tbody></table></div></section>
