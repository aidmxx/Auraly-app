import { createParticipant, logout } from "@/app/actions";
...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{created?:string;deleted?:string}> }) {
  ...
  return <main className="app-shell"><header className="topbar"><div><strong>Auraly Research Console</strong><span>Signed in as {admin.loginId}</span></div><form action={logout}><button className="link-button">Sign out</button></form></header>
    ...
    {(await searchParams).deleted&&<div className="success-banner">Participant account and all associated data were permanently deleted.</div>}
    ...
    <section className="panel"><div className="section-head"><div><h2>Participant progress</h2><p className="muted">Times are shown in Sydney local time. Deleting a participant permanently removes their account and all associated study data.</p></div><div className="export-links"><a href="/api/admin/export?format=csv">Export CSV</a><a href="/api/admin/export?format=json">Export JSON</a></div></div><div className="table-wrap"><table><thead><tr><th>ID</th><th>Condition</th><th>Status</th><th>Started</th><th>Submitted</th><th>Time</th><th>Words</th><th>AI uses</th><th>Drafts</th><th>Account</th></tr></thead><tbody>{rows.map(r=><tr key={String(r.id)}><td><strong>{String(r.login_id)}</strong></td><td>{String(r.condition_code)}</td><td><span className={`status ${r.status}`}>{String(r.status).replace("_"," ")}</span></td><td>{r.started_at?formatSydneyTime(r.started_at):"—"}</td><td>{r.submitted_at?formatSydneyTime(r.submitted_at):"—"}</td><td>{r.completion_seconds?`${Math.round(Number(r.completion_seconds)/60)} min`:"—"}</td><td>{Number(r.final_word_count)||"—"}</td><td>{Number(r.interactions)}</td><td>{Number(r.drafts)}</td><td><DeleteParticipantButton userId={String(r.id)} loginId={String(r.login_id)} /></td></tr>)}</tbody></table></div></section>
    <section className="panel"><div className="section-head"><div><h2>AI run usage</h2><p className="muted">Token counts and response time for each newly generated AI interaction. Older runs created before usage tracking show only in the writing archive.</p></div></div><div className="table-wrap"><table><thead><tr><th>Participant</th><th>Interaction</th><th>Provider</th><th>Model</th><th>Input tokens</th><th>Output tokens</th><th>Total tokens</th><th>Response time</th><th>Generated</th></tr></thead><tbody>{aiRuns.length ? aiRuns.map(run=><tr key={String(run.id)}><td><strong>{String(run.login_id)}</strong></td><td>{Number(run.sequence_no)}</td><td>{String(run.provider)}</td><td>{String(run.model)}</td><td>{run.input_tokens==null?"—":Number(run.input_tokens)}</td><td>{run.output_tokens==null?"—":Number(run.output_tokens)}</td><td>{run.total_tokens==null?"—":Number(run.total_tokens)}</td><td>{(Number(run.duration_ms)/1000).toFixed(1)} sec</td><td>{formatSydneyTime(run.created_at)}</td></tr>) : <tr><td colSpan={9}>No token-tracked AI runs yet.</td></tr>}</tbody></table></div></section>
